...
def _card_payload(slot, person, emp_id, track, roster=None, to_fill=None, candidates=None,
    ...
    return {
        ...
        "locked": locked,
        "emp": emp_id,
        "draggable": bool(person) and track == "A" and not locked,
        "badges": badge_items,
        "roster": roster or [],
    }
...
def handle_org_event(event, data, slots):
    ...
    if person is None:
        return
    # 확정된 트랙의 인원은 더 이상 이동 불가 (JS에서도 막지만 이중 방어)
    confirmed = st.session_state.get("confirmed", {})
    if person["level"] != "직원" and confirmed.get("A"):
        return
    if person["level"] == "직원" and confirmed.get("B"):
        return
